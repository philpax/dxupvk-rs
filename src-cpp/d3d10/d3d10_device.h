...
namespace dxvk {
  ...
  class D3D10Device final : public ComObject<ID3D10Device1> {
  ...
  public:
    ...
    ID3D11Device* GetD3D11Iface() {
      return m_device.ref();
    }

    ID3D11DeviceContext* GetD3D11ContextIface() {
  ...
  IDX10* LookupFromD3D11(IDX11* address)
  {
    ...
    {
      Logger::err("Failed to lookup D3D11 interface.");
      return nullptr;
    }
  ...
  IDX10* LookupOrCreateFromD3D11(IDX11* address, D3D10Device* device)
  {
    UINT DataSize = sizeof(void*);
    IDX10* returnAddress;
    if (!FAILED(address->GetPrivateData(__uuidof(D3D10Map), &DataSize, &returnAddress)))
    ...
    return new DX10(address, device);
  }
